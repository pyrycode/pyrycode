...
- **One mutation seam, multiple drivers** — `Pool.RotateID` is the single place that swaps a session's in-memory id and persists. Startup reconciliation drives it from a directory scan; the next ticket (live `/clear` detection) will drive it from an fsnotify event. Defining the seam once with the lock-and-save contract baked in means the second driver is a goroutine that calls a function, not a redesign.
