...
- **Existing harnesses with a bare `bytes.Buffer` for `cmd.Stderr` (`AttachHarness`, `Harness`, `StdioAttachClient`) work today only because their stderr reads happen post-`Wait`.** A future test that adds a mid-test stderr probe to one of those harnesses needs the same shim. The existing #161 `StdioAttachClient.Stderr` would race if its proof-of-life weren't `t.Skip`'d; left untouched per scope discipline pending #167.
...
- **Mitigation: the real CLI path needs at least one e2e proof per dispatch surface.** Unit tests against `parseFooArgs` cover the verb-flag grammar in isolation but say nothing about whether `splitArgs` + `parseClientFlags` + dispatch will route a real argv to that grammar at all. `internal/e2e` is the right home: it builds the binary, runs the real argv pipeline, and surfaces dispatch-level failures the unit tests structurally cannot.
