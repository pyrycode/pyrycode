...
|---|---|---|---|
| `v0.11.0` | mobile API: net surface + relay roundtrip | 2026-05-14 | Mobile wire protocol v1 net handlers — `register_push_token` (#319), `send_message` (#322) + e2e (#323), `list_conversations` (#303), `assistant-turn → message` envelope bridge (#311). First-frame auth gate with WS 4401 close (#308) + per-conn auth slot on `*dispatch.Conn` (#318). Supervisor `WriteUserTurn` + `conversation_id` cursor (#312). Relay roundtrip e2e — appendix-flow happy path (#297). Dispatcher race fix — `gateRun` split into `gateStarted`/`gateCompleted` (#311). `pyry agent-run` subcommand skeleton (#337). 282 commits since v0.10.1 |
