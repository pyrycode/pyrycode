...
Pyrycode is a process supervisor that keeps a Claude Code session alive across crashes and reboots. The single-session supervisor (Phase 0) and multi-session routing (Phase 1) have shipped, and remote access is live — the Pyrycode-Relay runs at `pyrycode-relay.pyryco.de` (binary↔relay WSS, Noise_IK E2E crypto, QR pairing). Channels integration and voice remain on the roadmap; see [`plan.md`](../../plan.md) for the canonical phase numbering.
...
Fields: ClaudeBin (path to claude), WorkDir (child's cwd), ResumeLast (use --continue after first run), ClaudeArgs (pass-through args), Bridge (optional service-mode I/O mediator), Logger (*slog.Logger), backoff params (Initial, Max, Reset durations), ValidateConversation (optional `func(id string) error` consulted by `WriteUserTurn` before mutation; production wiring in `internal/sessions/pool.go` synthesises a closure over `conversations.Registry.Get` that returns `conversations.ErrConversationNotFound` on miss — `internal/supervisor` never imports `internal/conversations`, the sentinel travels through the closure verbatim; nil skips validation, #312), ResolveTranscript (optional `func(ctx) (path string, size int64, err error)` — when set, `WriteUserTurn` confirms a turn committed by observing the resolved claude transcript *grow* rather than trusting tui-driver's `Committed` chip heuristic; production wiring in `internal/sessions/pool.go` sets it on the bootstrap path only, gated on `ClaudeSessionsDir != ""` (the `newTranscriptResolver` closure over `mostRecentJSONL`); nil preserves the #594 `Committed` behaviour, #668).

### `supervisor.Bridge`

Service-mode I/O mediator. A single `Bridge` instance persists across child restarts; the supervisor brackets each `runOnce` iteration with `Bridge.BeginIteration()` / `Bridge.EndIteration()` so the input pump goroutine terminates cleanly per iteration instead of leaking and racing the next one for queued attach-client bytes. Input path: `chan []byte` + per-iteration cancel (`Bridge.Read` selects between an incoming chunk and EOF on iteration end — Go's `select` non-determinism preserves any in-flight chunk for the next iteration). Output path: `Write` fans the same bytes to **two independent heads** — `output` (the local `pyry attach` head, at-most-one, bound by `Attach`, `ErrBridgeBusy` on a second attach) and `outputObserver` (the phone observer head, a non-owning tap set by `cmd/pyry/assistant_turn_v2.go`) — then returns; it never returns an error. Either head may be nil (that head discards). The two seams are independent by construction: neither reads nor mutates the other's field, so neither head can corrupt the other, and a faulting `output` write is swallowed without starving the observer. This is the **Phase-1 two-heads model** (ADR 025, #595) — `Attach` is deliberately *not* re-seated onto a separate mirror seam; this at-most-one-attacher + observer-tap shape *is* the intended two-heads surface. Since #593 the supervisor feeds `Write` from `Session.MirrorOutput()` (drop-newest, buffer 256) rather than `io.Copy`'ing a raw PTY master, so a stalled bridge consumer can no longer wedge claude — tui-driver's internal reader drains the PTY independently. Resize seam (#593): `SetResizer(resizer)` registers a per-iteration single-method `resizer` delegate (`Resize(rows, cols uint16) error`, satisfied by `*tuidriver.Session`) under a leaf-only `ptyMu`; `Resize(rows, cols uint16)` forwards to it (silently no-ops between iterations). `runOnce` clears the registration with `SetResizer(nil)` **before** `EndIteration` so a racing `Resize` forwards to nil rather than a closing session — and `Session.Resize` returns an error on a closed FD instead of touching a dangling `*os.File`, structurally defusing the old EBADF race. See [ADR 007](../decisions/007-bridge-iteration-boundaries.md), [ADR 008](../decisions/008-bridge-resize-seam.md), [codebase/593.md](../codebase/593.md), [codebase/595.md](../codebase/595.md).
...
- `WriteUserTurn(ctx context.Context, id string, payload []byte) error` (#312, reliable delivery #594) — delivers a user-turn payload to the supervised child along a caller-tagged `conversation_id` and returns `nil` **only** when claude confirms the turn committed. Runs `Config.ValidateConversation` first (a non-nil result propagates verbatim, typically `conversations.ErrConversationNotFound`); on accept stamps the cursor under `convMu` **before** delivery — including when delivery then fails — so concurrent `CurrentConversation()` readers never see a stale cursor; cursor is NOT mutated on validation refusal. Delivery captures the live `Session` under `sessMu`, releases the lock, then runs the ready-gate (`Session.WaitReady`) + commit-confirm (`Session.DeliverPrompt`) on the captured pointer via the unexported set-once `deliverFn` seam (`JSONLPath` always left empty — the bootstrap session spawns with `--continue`, not `--session-id`, so no stable claude UUID exists, and tui-driver's appearance-based `JSONLPath` signal is unreliable under resume anyway). The commit-confirm has **two modes** keyed on `Config.ResolveTranscript` (#668): when nil (foreground / tests) it trusts `DeliverResult.Committed` (the #594 chip heuristic, sufficient with the idle gate in front); when set (the relay-driven bootstrap path) it **ignores `Committed`** and confirms deterministically by polling the resolved transcript for *growth* — a real commit appends to claude's JSONL, so the resolved file getting bigger (or a `/clear`-rotated newer file appearing) past a post-`WaitReady` baseline is the signal; no growth within `transcriptConfirmTimeout` (10s, capped by the caller's ctx) → `ErrTurnNotCommitted`. The growth mode exists because the chip heuristic *false-acks* the short single-line first mobile turn when it races claude's ~7.5s `--continue` restart (no `[Pasted text]` chip renders for a typed prompt). Since #594 there is **no silent drop**: no live session → `ErrNoLiveSession`, an uncommitted/wedged turn → `ErrTurnNotCommitted`, and a ready-gate timeout (claude busy past the caller's ctx) → wrapped `context.DeadlineExceeded` — all return non-nil so the relay `send_message` handler reports a retryable failure to the phone instead of a false ack. Every non-nil return keeps the stable `"supervisor: write user turn:"` wrap prefix. See [codebase/594.md](../codebase/594.md), [codebase/668.md](../codebase/668.md).
...
- `ScreenSnapshot() (text string, live bool)` (#618) — renders the current claude screen to plain text for the relay's on-demand `request_snapshot` (ADR 025's parser-independent live-view escape hatch). Captures the live `Session` under `sessMu` (mirroring `WriteUserTurn`'s pattern), releases the lock, then renders `tuidriver.Render(sess.Snapshot(), 0, 0)` **inside the tui-driver seal** — the raw bytes are consumed in the same expression and never named in pyrycode, so `cmd/substrate-guard` stays green. `live == false` (text `""`) when no child is attached. Total (no error path), non-blocking (a bounded in-memory render). Reached from the v2 relay via the `relay.ScreenSnapshotter` consumer-declared interface that `*Supervisor` satisfies — `internal/supervisor` stays unaware of the relay. See [codebase/618.md](../codebase/618.md).

Internal state: `convMu` + `currentConvID` (the cursor, #312) and `sessMu` + `sess *tuidriver.Session` (the per-iteration hosted session, re-keyed from #312's `ptmx *os.File` by #593). Both leaf-only; never nested with each other or with the pre-existing `mu`/`state` pair. `runOnce` brackets each iteration with private `setSession(sess)` / `setSession(nil)`, mirroring `Bridge.SetResizer`'s register/clear pair — `setSession(nil)` runs **before** `sess.Close()`. Since #594 `WriteUserTurn` captures the `Session` under `sessMu` then **releases the lock before delivering** (a `WaitReady`+`DeliverPrompt` can run for seconds and must not block teardown), so this ordering is relaxed: a racing `WriteUserTurn` that captured the pointer just before the clear may still deliver against a closing session — but only into tui-driver's teardown-safe PTY-error path (no panic, no dangling FD, same property the #593 `Resize` EBADF defusal established), which surfaces as a loud failure, never a crash or a false ack. The `WaitForPTY` readiness seam (freshening `sessReadyCh`, [ADR 023](../decisions/023-activate-waits-pty-readiness.md)) is re-keyed to "Session is live" with byte-identical choreography. See [codebase/593.md](../codebase/593.md), [codebase/594.md](../codebase/594.md).
...
|--------|---------|----------------|
| `pyrycode/tui-driver` | Hosts claude as a `Session` (PTY spawn, sealed raw-byte `MirrorOutput`, `AttachInput`, `Resize`, `Wait`/`Close`) — the supervisor's host loop since #593, and `internal/agentrun/ptyrunner`'s since #471 | Single home for all claude screen knowledge (substrate seal); no stdlib equivalent. See [ADR 025](../decisions/025-mobile-remote-head-interactive-session.md). |
...
## Beyond Phase 0 — landed work and roadmap

The entries below track the multi-session pool (Phase 1) and the mobile/relay stack (Phase 3 in the codebase's foundation tickets) as they landed, each annotated with its shipped status. The Pyrycode-Relay is **LIVE at `pyrycode-relay.pyryco.de` since 2026-05-29** — the relay client, Noise_IK crypto, QR pairing, WSS transport, and per-conn dispatch listed here are all shipped, not future. Voice (WebRTC/STT/TTS) is the only item in this list that is still unimplemented. For the authoritative phase numbering and what each phase covers, [`plan.md`](../../plan.md) is the source of truth — the per-ticket notes below describe what landed, not a planning sequence.
...
- **Phase 3 wiring (#677, landed; EPIC #672):** `create_conversation` now mints and binds a dedicated claude session per conversation instead of every discussion sharing the single bootstrap claude. The handler (`internal/relay/handlers/create_conversation.go`) eagerly calls `sessions.Pool.Create` (AC#1 forces eager-over-lazy — the row must carry a non-empty `CurrentSessionID` pointing at a pool session as soon as the create frame is handled), records the bound id on the existing `Conversation.CurrentSessionID` field, and eager-Saves (the binding round-trips on registry reload). A narrow `handlers.SessionCreator` interface is threaded through `startRelay`/`startRelayV2` and adapted from `*sessions.Pool` at the `cmd/pyry` boundary (`sessionMinter`, mirroring `poolResolver`), keeping `handlers/` free of `internal/sessions`. The session spawns in the daemon's already-trust-marked shared workdir (`buildSession`'s `tpl.WorkDir`); the phone-influenced `Cwd` is structurally excluded from the spawn path (AC#4 — the handler passes only `(mintCtx, string(id))`). Distinct per-conversation working directories + their trust/boundary validation, and a per-operator create quota, are deferred #672 follow-ups. See [features/conversation-session-binding.md](../features/conversation-session-binding.md), [codebase/677.md](../codebase/677.md), [ADR 025](../decisions/025-mobile-remote-head-interactive-session.md).
...
- **Channels** — pyrycode-owned Discord/Telegram clients, inbound event routing (Phase 2 in [`plan.md`](../../plan.md)).
- **Cross-cutting services in-process** — knowledge capture, memsearch, supervised cron runner (Phase 4 in [`plan.md`](../../plan.md)).
- **Voice** — WebRTC via `pion/webrtc`, STT/TTS pipeline (Phase 6 in [`plan.md`](../../plan.md)). The only item in this document's beyond-Phase-0 list that is still unimplemented.

Remote access (relay server, Noise E2E crypto, QR pairing — Phase 5 in [`plan.md`](../../plan.md)) is **live**, not future: the relay client, transport, Noise wrapper, pairing, and dispatch shipped in the foundation tickets above and the relay itself runs at `pyrycode-relay.pyryco.de` as of 2026-05-29.
