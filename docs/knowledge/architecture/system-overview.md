...
- `WriteUserTurn(id string, payload []byte) error` (#312) — delivers a user-turn payload to the supervised child along a caller-tagged `conversation_id`; runs `Config.ValidateConversation` first (a non-nil result propagates verbatim, typically `conversations.ErrConversationNotFound`); on accept updates the cursor under `convMu` **before** writing to the session (via `Session.AttachInput` since #593; the raw fire-and-forget contract is unchanged — the `Session.DeliverPrompt` ready-gate/commit-confirm rewrite is #594) so concurrent `CurrentConversation()` readers never see a stale cursor; cursor is NOT mutated on validation refusal; no live session (between iterations / pre-spawn) drops the bytes silently and returns `nil` (matches `Bridge.Write`'s discard-on-unattached behaviour); write failures wrap with stable `"supervisor: write user turn:"` prefix.
