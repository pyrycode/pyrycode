...
`schedule: cron "13 6 * * *"` (06:13 UTC daily, off-peak from main CI) + `workflow_dispatch`. Steps: checkout → setup-go → `npm install -g @anthropic-ai/claude-code` → `go build -o pyry ./cmd/pyry` → `./pyry agent-run --self-check` with `ANTHROPIC_API_KEY` from repo secrets. Red badge IS the failure signal; cost is one short claude turn per day, ≤ $0.01. The exit-code contract (`0` PASS, non-zero FAIL/inconclusive) is unchanged from #336 / #375 / #473; the workflow now structurally validates the production ptyrunner path instead of the (no-longer-production) streamrunner path. No workflow-file edit required for the cutover — the contract is at the verb boundary, not in the workflow plumbing.
