...
`Emit` counts by **transition**: it tracks the previous assistant entry's id in `e.lastAssistantMsgID` and increments `numTurns` only when `agentrun.IsNewLogicalTurn(id, e.lastAssistantMsgID)` reports a new turn (the current entry's `message.id` is empty or differs). Intervening non-assistant entries (`user`/`tool_result`) fall outside the `entry.Type == "assistant"` block, so they never touch `lastAssistantMsgID` and never split a turn whose lines straddle them. Transition-counting equals distinct-id counting because claude completes one assistant message before starting the next — no `A,B,A` interleaving has been observed, so no seen-set is kept (a code comment on the shared helper documents the assumption).

**Shared predicate (#574).** The boundary test is the exported `agentrun.IsNewLogicalTurn(currentID, lastID string) bool` (pure over two strings; see [agentrun-package.md](agentrun-package.md)), not an inline expression. The budget `Counter`'s `--max-turns` enforcement (`internal/agentrun/budget`) calls the **same** function, so the reported `num_turns` and the enforced budget cannot drift on what a turn is — a single source of truth rather than two copies kept identical by hand. #573 introduced the predicate inline here; #574 extracted it and made budget the second (and only other) caller.

**empty-id floor:** an entry with no `message.id` (`Message == nil` or `ID == ""`) is ungroupable and counted as its own turn, preserving the pre-#573 per-entry behaviour for id-less entries. Real claude always emits a non-empty id; empty ids appear only in synthetic/malformed entries — this is why `TestReadUsage_*` (single id-less entry → `num_turns == 1`) stays green. The fix is the consumer-side `msg_id` grouping tuidriver deliberately leaves to consumers (`IsEndTurn`/`AssistantText` are per-entry). See [`codebase/573.md`](../codebase/573.md). The budget Counter (`internal/agentrun/budget`) once double-counted identically on the enforcement path; #574 aligned it to the same rule by extracting the shared `agentrun.IsNewLogicalTurn` both call (see [`codebase/574.md`](../codebase/574.md)).
...
- [#574](../codebase/574.md) — extracted the inline turn-boundary test into the shared `agentrun.IsNewLogicalTurn` and applied it to the budget Counter's `--max-turns` enforcement, so reported `num_turns` and the enforced budget cannot drift.
