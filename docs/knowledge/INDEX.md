...
| [e2e-harness.md](features/e2e-harness.md) | `internal/e2e` (build tag `e2e` or `e2e_install`) — `Harness` + `Start(t)` spawn pyry in temp HOME, `Harness.Run(t, verb, args...)` auto-injects socket, `RunBare(t, args...)` drives daemon-free verbs (`version`, negative-path `status`); covers `stop`/`logs`/`version`/`status` (running + stopped); SIGTERM/SIGKILL teardown, leak verification via re-exec |
