...
| [e2e-harness.md](features/e2e-harness.md) | `internal/e2e` (build tag `e2e` or `e2e_install`) — `Harness` + `Start(t)` / `StartIn(t, home)` spawn pyry in temp / caller-supplied HOME, `StartExpectingFailureIn(t, home)` for failed-start assertions, `Harness.Run(t, verb, args...)` auto-injects socket, `RunBare(t, args...)` drives daemon-free verbs, `Harness.Stop(t)` graceful mid-test teardown (idempotent with `t.Cleanup`); covers `stop`/`logs`/`version`/`status` (running + stopped), three restart-survival proofs (active, evicted-state, `lastActiveAt`), and corrupt-registry fail-loud at the binary boundary; SIGTERM/SIGKILL teardown, leak verification via re-exec |
