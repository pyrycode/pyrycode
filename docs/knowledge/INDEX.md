...
| [e2e-harness.md](features/e2e-harness.md) | `internal/e2e` (build tag `e2e` or `e2e_install`) — `Harness` + `Start(t)` spawn pyry in temp HOME, `Harness.Run(t, verb, args...)` drives CLI verbs with auto-injected socket, SIGTERM/SIGKILL teardown, leak verification via re-exec |
| [install-e2e.md](features/install-e2e.md) | `internal/e2e/install_linux_test.go` (build tag `linux && e2e_install`) — `pyry install-service` × `systemctl --user` round-trip + bug-#19 PATH inheritance regression guard + cleanup-on-fatal re-exec verification |
