...
| [e2e-harness.md](features/e2e-harness.md) | `internal/e2e` (build tag `e2e` or `e2e_install`) — `Harness` + `Start(t)` / `StartIn(t, home)` spawn pyry in temp / caller-supplied HOME, `Harness.Run(t, verb, args...)` auto-injects socket, `RunBare(t, args...)` drives daemon-free verbs, `Harness.Stop(t)` graceful mid-test teardown (idempotent with `t.Cleanup`); covers `stop`/`logs`/`version`/`status` (running + stopped) and the restart-survival proof (`TestE2E_Restart_PreservesActiveSessions`); SIGTERM/SIGKILL teardown, leak verification via re-exec |
