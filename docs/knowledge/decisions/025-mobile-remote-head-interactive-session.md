...
- `request_snapshot` — `{conversation_id}`. Asks for a `screen_snapshot` of the current screen. Always available as a manual "show the literal screen" action, not only on a stall; the stall path surfaces the same action prominently.
...
**Only answering permission-class prompts is gated.** The per-device gate applies solely to *answering* a permission, trust, or destructive modal. Everything else a paired phone does is its normal capability and is NOT gated: viewing the structured event stream, requesting an on-demand `screen_snapshot`, sending messages, interrupting, and dequeuing. Read-only viewing in particular exposes nothing beyond what the phone already receives as events, so peeking at the literal screen needs no grant. This keeps the snapshot and ordinary control frictionless on any paired phone, while only the permission-granting surface sits behind the per-device flag.
...
- **The live-view escape hatch — an always-available screen snapshot.** The phone can ask for a one-shot text picture of the current screen at any time, not only on a stall: a `request_snapshot` control message → the daemon calls tui-driver's `Render` / `Snapshot` → a `screen_snapshot` typed event carrying the rendered text. The phone renders that text. It is NOT a raw byte stream and NOT raw terminal control codes, so the "phone never receives raw screen bytes" invariant holds and the substrate seal holds: tui-driver renders the snapshot inside the seal, and pyrycode forwards it as an opaque event payload, never as a source literal. This is a first-class "show me what is actually on screen right now" action, exposed manually on the phone; the stall path surfaces the same action prominently and is the concrete meaning of "open the live view." Because it depends on no parser, it survives any parser break — which is why it is the floor when everything structured is brittle. The snapshot is fetched **on-demand** by default (rendered only when the user opens the view, with optional refresh-by-re-request); whether to ALSO push it eagerly on `stall_detected` is a Phase 2/3 sub-decision.
...
**Phase 2 — structured streaming.** Epic pyrycode **#596**, *blocked-by #594*. Decomposes into: tui-driver tool-use/tool-result JSONL helpers + stall signal; pyrycode interactive event types + capability negotiation; the event-stream bridge (replaces #589's coarse fan-out) + delta coalescing; the always-available screen-snapshot capability (`request_snapshot` / `screen_snapshot`, parser-independent, low complexity since `Render` already exists); backpressure/droppable-delta policy; mid-turn reconnect replay; mobile thinking indicator + tool-use timeline + snapshot action + reconnect client. Re-point mobile #336 / #337 onto the event-stream-bridge child.
